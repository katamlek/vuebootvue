    ...
    <div class="container">
      <table class="table">
        ...
        <tr v-for="skill in skills" v-bind:key="skill.id">
          ...
          <td>{{skill.name}}</td>
          <td>{{skill.level}}</td>
          <td><button class="btn btn-success" v-on:click="updateSkillClicked(skill.id)">Update</button></td>
          <td><button class="btn btn-warning" v-on:click="deleteSkillClicked(skill.id)">Delete</button></td>
        </tr>
        </tbody>
        <div class="row">
